...
(function (window){
    ...
    Sakri.CanvasTextProperties.prototype.setFontSize = function(fontSize){
        if(fontSize && fontSize.indexOf && fontSize.indexOf("px")>-1){
            ...
            fontProperites.fontSize = isNaN(size) ? 24 : size;
            return;
        }
        this.fontSize = isNaN(fontSize) ? 24 : fontSize;
    };
...
function commitResize(){
    ...
    var textInputSpanY = (canvas.height - canvas.height*.85)/2 + 15;
...
function stageTooSmallHandler(){
    ...
    function startDemo(){
        ...
        var logoText = "Flappium";
        if(!logoCanvas){
            logoCanvas = document.createElement("canvas");
            logoCanvasBG = document.createElement("canvas");
        }
        createLogo("Flappium", logoCanvas, logoCanvasBG);
        if(!gameOverCanvas){
    ...
    function renderGameOver(){
        ...
        var instruction = "Cliquez pour jouer";
        context.font = "bold normal 24px sans-serif";
    ...
    function renderTube(tube){
        ...
        tube.bottomRect.height = groundGraphicRect.y - tube.bottomRect.y - 1;
